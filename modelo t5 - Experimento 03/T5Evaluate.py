...
parser = argparse.ArgumentParser(description="Code for running evaluation")
...
path = args.model_output #"results_exp3"
last_ckpt = 0
last_ckpt = max([int(f.replace("checkpoint-","")) for f in os.listdir(path)])
with open(os.path.join(path, "checkpoint-" + str(last_ckpt), "trainer_state.json")) as f:
    data = json.load(f)
print("Best checkpoint:", data["best_model_checkpoint"])
model_name = data["best_model_checkpoint"]

for f in os.listdir(path):
    fname =  os.path.join(path, f)
    if fname != data["best_model_checkpoint"]:
        try:
            print("remove", fname)
            shutil.rmtree(fname)
        except OSError as e:
...
def compute_metrics_v2(eval_preds):
    ...
    print(f"bleu_preds {preds[0]}")
    print(f"bleu_refs {refs[0]}")
